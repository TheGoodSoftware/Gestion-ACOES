...
CREATE TABLE IF NOT EXISTS `mydb`.`apadrinar` (
  ...
  INDEX `fk_NIÑO_has_USUARIO_USUARIO1` (`USUARIO_idUsuario` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `mydb`.`asociacion` (
  ...
  PRIMARY KEY (`Nombre`),
  UNIQUE INDEX `idAs_UNIQUE` (`Nombre` ASC) VISIBLE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE IF NOT EXISTS `mydb`.`contenedor` (
  ...
  UNIQUE INDEX `idCont_UNIQUE` (`idCont` ASC) VISIBLE,
  INDEX `fk_CONTENEDOR_USUARIO2` (`USUARIO_idUsuario` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `mydb`.`donacion` (
  ...
  UNIQUE INDEX `idDon_UNIQUE` (`idDon` ASC) VISIBLE,
  INDEX `fk_DONACIÓN_GESTIÓN ECONÓMICA1` (`GESTION ECONOMICA_idBalance` ASC) VISIBLE,
  INDEX `fk_DONACIÓN_USUARIO1` (`USUARIO_idUsuario` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `mydb`.`gasto` (
  ...
  INDEX `fk_GASTO_GESTIÓN ECONÓMICA1` (`GESTION ECONOMICA_idBalance` ASC) VISIBLE,
  INDEX `fk_GASTO_USUARIO2` (`USUARIO_idUsuario` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `mydb`.`gestionEconomica` (
  ...
  UNIQUE INDEX `idBalance_UNIQUE` (`idBalance` ASC) VISIBLE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE IF NOT EXISTS `mydb`.`nino` (
  ...
  PRIMARY KEY (`idNen`),
  UNIQUE INDEX `idNen_UNIQUE` (`idNen` ASC) VISIBLE,
  INDEX `fk_NINO_PROYECTO1` (`PROYECTO_idProy` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `mydb`.`notas` (
  ...
  UNIQUE INDEX `idNotasNen_UNIQUE` (`idNotasNen` ASC) VISIBLE,
  INDEX `fk_notas_nino1_idx` (`nino_idNen` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `mydb`.`objeto` (
  ...
  INDEX `fk_OBJETO_CONTENEDOR1` (`CONTENEDOR_idCont` ASC) VISIBLE,
  INDEX `fk_OBJETO_USUARIO2` (`USUARIO_idUsuario` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `mydb`.`proyecto` (
  ...
  PRIMARY KEY (`idProy`),
  UNIQUE INDEX `idProy_UNIQUE` (`idProy` ASC) VISIBLE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE IF NOT EXISTS `mydb`.`responsableDe` (
  ...
  INDEX `fk_USUARIO_has_PROYECTO_PROYECTO1` (`PROYECTO_idProy` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `mydb`.`rolesPermisos` (
  ...
  INDEX `fk_PERMISO_has_ROL_ROL1` (`ROL_idRol` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `mydb`.`usuario` (
  ...
  INDEX `fk_USUARIO_ASOCIACIÓN1` (`ASOCIACION_Nombre` ASC) VISIBLE,
